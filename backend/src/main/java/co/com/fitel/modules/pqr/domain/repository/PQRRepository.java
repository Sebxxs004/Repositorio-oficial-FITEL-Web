...
public interface PQRRepository extends JpaRepository<PQR, Long> {
    ...
    @Query(value = "SELECT p.*, " +
           ...
           "FROM pqr p WHERE (p.cun LIKE CONCAT('%', MOD(:anio, 100), LPAD(CONVERT(:consecutivo, CHAR), 10, '0')) OR p.cun LIKE CONCAT('%', MOD(:anio, 100), LPAD(CONVERT(:consecutivo, CHAR), 6, '0'))) COLLATE utf8mb4_unicode_ci", 
           nativeQuery = true)
